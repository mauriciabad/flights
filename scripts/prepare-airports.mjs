...
// Issue #129 kept a marketed-city table in this file so "Paris" would find Beauvais.
// Issue #136 asked the same question about what to DISPLAY, and rather than keep a
// second list in agreement with this one, both now live in
// src/lib/data/airport-city-names.ts: one table, read by `toAirport` for the name on
// screen and by the search index for the names a traveller types. That file's header
// has the reasoning, including why a marketed name (Girona sold as "Barcelona") stays
// searchable without ever being printed.
//
// So this script is back to the job it started with, trimming the 12 MB OurAirports CSV
// to the rows and fields this app ships, unedited. Re-running it can no longer drop an
// editorial decision on the floor, because it no longer holds one.
...
function toCompactAirport(row) {
	...
	return {
		...
		// when OurAirports has none for this airport. `airport-city-names.ts` adds this
		// app's own alternates on top at load time (issue #136); none are baked in here.
		...(keywords ? { keywords } : {})
	};
}
