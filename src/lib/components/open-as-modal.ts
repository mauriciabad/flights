 ...
 * disagree, so it was deleted (#448) rather than copied here. The other half of the same
 * story is real. A click on a non-focusable descendant DOES land on the dialog, because a
 * modal dialog is the focus scope. `app.css`'s `dialog:focus-visible` rule is where that is
 * handled, for every dialog at once.
 *
