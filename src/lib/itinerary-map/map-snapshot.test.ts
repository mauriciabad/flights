...
import { cameraForFrame, snapshotKey } from './map-snapshot.svelte';
...
describe('snapshotKey', () => {
	it('gives one key to two cards looking at the same window', () => {
		...
		expect(snapshotKey(frame, 120, 88, 2, 'dark')).toBe(snapshotKey({ ...frame }, 120, 88, 2, 'dark'));
	});

	it('separates the things that change the pixels', () => {
		const frame = frameOf(2.0785, 40.1, 2.1734, 40.2);
		const base = snapshotKey(frame, 120, 88, 2, 'dark');
		expect(snapshotKey(frame, 120, 88, 2, 'light')).not.toBe(base);
		expect(snapshotKey(frame, 240, 88, 2, 'dark')).not.toBe(base);
		expect(snapshotKey(frame, 120, 176, 2, 'dark')).not.toBe(base);
		expect(snapshotKey(frame, 120, 88, 1, 'dark')).not.toBe(base);
		expect(snapshotKey(frameOf(2.0785, 40.1, 2.1734, 40.3), 120, 88, 2, 'dark')).not.toBe(base);
	});
	...
	it('rounds the bounds, so two routes to the same window share a capture', () => {
		...
		expect(snapshotKey(nudged, 120, 88, 2, 'dark')).toBe(snapshotKey(frame, 120, 88, 2, 'dark'));
	});
});
