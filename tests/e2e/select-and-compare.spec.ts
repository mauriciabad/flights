...
test.describe('select and compare (issues #103/#104)', () => {
	test('selecting real results carries them into the comparator, and a picker change updates the total', async ({
		...
		await page.context().route('https://services-api.ryanair.com/**', async (route) => {
			...
			if (dep === 'BCN' && (arr === 'VIE' || !arr)) {
				fares = [
					...
					ryanairFare({ dep: 'BCN', arr: 'VIE', depDate: '2026-10-01T16:30:00', arrDate: '2026-10-01T18:45:00', price: 54.49, flightNumber: 'FR1003' })
				];
		...
		await expect(totalPriceRow).toContainText('€84.99'); // 39.99 (cheaper outbound) + 45.00 onward
		...
		const alternativeRow = outboundPicker.locator('.picker-row', { hasText: '€54.49' });
		...
		await expect(totalPriceRow).toContainText('€99.49'); // 54.49 + 45.00
	});
});
